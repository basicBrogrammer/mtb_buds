Doorkeeper.configure do
  ...
  # https://codingitwrong.com/2018/03/12/setting-up-oauth-in-an-ember-rails-app.html
  resource_owner_from_credentials do
    user = User.find_for_database_authentication(:email => params[:username]) 

    if user && !user.active_for_authentication?
      raise Doorkeeper::Errors::DoorkeeperError.new('unconfirmed')
    elsif user && user.valid_for_authentication? { user.valid_password?(params[:password]) }
      user
    else
       raise Doorkeeper::Errors::DoorkeeperError.new('invalid')
    end
  end
